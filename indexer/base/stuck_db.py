...
class StuckDBTable(BaseDbTable):
    ...
    async def _set_obj_list(self, ctx: DbTxCtx, neon_block: NeonIndexedBlockInfo, obj_list: Sequence[dict]) -> None:
        await self._update_row(ctx, self._delete_query, _ByFinalized(neon_block.is_finalized))

        if not obj_list:
            return

        json_data = _ObjDictList(root=list(obj_list)).to_json()
        await self._insert_row(ctx, _RecordWithFinalized(neon_block.stuck_slot, json_data, neon_block.is_finalized))

    async def get_obj_list(self, ctx: DbTxCtx, is_finalized: bool) -> tuple[int | None, Sequence[dict]]:
