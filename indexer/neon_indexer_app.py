...
class NeonIndexerApp:
    ...
    async def _init_chain_id(self) -> None:
        ...
        _LOG.debug("default ChainID: %s, layer0 ChainID: %s", self._def_chain_id, self._layer0_chain_id)
    ...
    async def _run_indexing(self) -> None:
        assert self._first_slot is not None
        start_slot = max(self._first_slot, self._cfg_start_slot)
        await self._db.set_start_slot(start_slot)
    ...
    @cached_property
    def _last_known_slot(self) -> int:
        return self._db.get_min_used_slot()

    @cached_property
    def _cfg_start_slot(self) -> int:
        if self._is_indexing_disabled:
            _LOG.debug("%s=%s, skip indexing...", self._cfg.start_slot_name, self._cfg.start_slot)
            return self._finalized_slot
        return self._get_cfg_start_slot()

    @cached_property
    def _is_indexing_disabled(self) -> bool:
        return self._cfg.start_slot == StartSlot.Disable

    @cached_property
    def _reindex_ident(self) -> str:
        _, value = self._get_cfg_reindex_start_slot()
        return value

    @cached_property
    def _cfg_reindex_start_slot(self) -> int | None:
        value, _ = self._get_cfg_reindex_start_slot()
        return value

    async def _clear_by_sol_first_slot(
        self,
        slot_range_list: Sequence[IndexerDbSlotRange],
    ) -> Sequence[IndexerDbSlotRange]:
        assert self._first_slot is not None
        base_slot = self._first_slot

        stop_idx = next((i for i, s in enumerate(slot_range_list) if s.stop_slot >= base_slot), len(slot_range_list))

        skip_slot_range_list, slot_range_list = slot_range_list[:stop_idx], slot_range_list[stop_idx:]
        for slot_range in skip_slot_range_list:
            _LOG.info(
                "skip the lost REINDEX range %s: Solana first slot (%s) > db.stop_slot (%s)",
                slot_range,
                self._first_slot,
    ...
    async def _init_first_slot(self, slot_range_list: Sequence[IndexerDbSlotRange]) -> None:
        base_slot = min(slot_range_list[0].start_slot if slot_range_list else 0, self._cfg_start_slot)
        self._first_slot = await SolNotEmptyBlockFinder(self._sol_client, start_slot=base_slot - 1).find_slot()
        _LOG.debug("first slot: %s", self._first_slot)
    ...
    async def _is_reindex_completed(self, slot_range_list: Sequence[IndexerDbSlotRange]) -> bool:
        ...
        if (last_reindex_ident == self._reindex_ident) and (not slot_range_list):
    ...
    async def _add_new_reindex_range_list(
        ...
        if (start_slot := self._cfg_reindex_start_slot) is None:
            return slot_range_list
        elif slot_range_list:
            start_slot = max(slot_range_list[-1].stop_slot, start_slot)

        assert self._first_slot is not None

        start_slot = max(self._first_slot, start_slot)
    ...
    async def _build_new_reindex_range_list(self, start_slot: int, avail_cnt: int) -> Sequence[IndexerDbSlotRange]:
        ...
        """
        if self._is_indexing_disabled:
            reindex_stop_slot = (await self._db.get_finalized_slot()) or self._finalized_slot
        else:
            reindex_stop_slot = self._cfg_start_slot

        total_len = reindex_stop_slot - start_slot + 1
        if total_len <= 0:
            return tuple()
        ...
        slot_range_list: list[IndexerDbSlotRange] = list()
        while start_slot < reindex_stop_slot:
            stop_slot = min(start_slot + range_len, reindex_stop_slot)

            block_finder = SolNotEmptyBlockFinder(self._sol_client, start_slot=start_slot, stop_slot=stop_slot)
    ...
    def _get_cfg_start_slot(self) -> int:
        ...
        """
        last_known_slot = self._last_known_slot
    ...
    def _get_cfg_reindex_start_slot(self) -> tuple[int | None, str]:
        ...
        """
        if not self._cfg.reindex_thread_cnt:
            _LOG.debug("skip reindexing: %s=%s", self._cfg.reindex_thread_cnt_name, self._cfg.reindex_thread_cnt)
            return None, ""
        ...
        if isinstance(reindex_ident, int):
            ...
            if self._cfg.start_slot not in (StartSlot.Latest, StartSlot.Disable):
                _LOG.error(
                    "skip reindexing: wrong value %s=%s, it is valid only for %s=(%s)",
                    self._cfg.reindex_start_slot_name,
                    StartSlot.Continue,
                    self._cfg.start_slot_name,
            ...
            start_slot = self._last_known_slot or self._finalized_slot
        ...
        _LOG.error("skip reindexing: wrong value %s=%s", self._cfg.reindex_start_slot_name, reindex_ident)
        return None, ""

    async def _run_reindexing(self) -> None:
        """
        ...
        await self._init_first_slot(slot_range_list)

        slot_range_list = await self._clear_by_sol_first_slot(slot_range_list)
    ...
    async def _wait_reindexing(self):
