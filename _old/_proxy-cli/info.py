...
class InfoHandler:
    ...
    def execute(self, args) -> None:
        if args.subcommand == 'solana-private-keys':
            ...
            self._sol_client_accounts_info(args)
        ...
        for key_info in key_info_list:
            ...
            neon_acct_dict: Dict[str, Dict[str, Any]] = {
                get_token_name(neon_acct.chain_id): dict(
                    neon_address=neon_acct.neon_address.to_checksum_address(),
                    neon_private=neon_acct.neon_address.private_key.to_hex(),
    ...
    def _get_neon_balance(self, neon_address: NeonAccount) -> Decimal:
    ...
    def _get_sol_balance(self, sol_pubkey: SolPubKey) -> Decimal:
        balance = self._sol_client.get_sol_balance(sol_pubkey)
