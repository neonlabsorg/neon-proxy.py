...
class NeonHandler:
    ...
    def init_args_parser(parsers) -> NeonHandler:
        ...
        n.sub_parser = n.root_parser.add_subparsers(
        ...
        n.withdraw_parser.add_argument(
            "dest_address", type=str, help="destination address for withdraw"
        )
        ...
        token_list = "|".join(
            token_name_list
            + [name + NeonHandler._percent_postfix for name in token_name_list]
        )
        n.withdraw_parser.add_argument(
            "type",
            type=str,
            default=NeonHandler._percent,
            nargs="?",
    ...
    def _get_neon_addr_dict(self) -> Dict[NeonAccount, Wei]:
        key_info_list = get_key_info_list()

        neon_acct_dict = {
            neon_acct.neon_address: self._proxy(neon_acct.chain_id).get_balance(
                Address(bytes(neon_acct.neon_address))
            )
    ...
    def _withdraw_neon(self, args) -> None:
        ...
        token_set = set(
            token_name_list
            + [name + self._percent_postfix for name in token_name_list]
            + [self._percent]
        )
        ...
        if a_type not in token_set:
            print(
                f'wrong type of amount type {a_type}, should be {", ".join(sorted(token_set))}',
                file=sys.stderr,
            )
            return
        elif amount <= 0:
            ...
            a_type == self._percent or a_type.endswith(self._percent_postfix)
        ):
            print(
                f"amount {amount} is too big, should be less or equal 100",
                file=sys.stderr,
            )
            return
        ...
        for neon_addr, balance in neon_acct_dict.items():
            ...
            token_balance_dict[token_name] = (
                token_balance_dict.get(token_name, 0) + balance
            )
        ...
        if a_type == self._percent:
            ...
            if check_balance < amount:
                print(
                    f"amount {amount} is too big, should be less than {check_balance}",
                    file=sys.stderr,
                )
                return
