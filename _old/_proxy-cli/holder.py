...
from common_neon import OpResInfo
from common_neon import NeonTxStage, NeonCreateHolderAccountStage, NeonDeleteHolderAccountStage
from common_neon.solana import SolTxListSender
...
class HolderHandler:
    ...
    def _create_holder_account(self, args) -> None:
        ...
        holder_info = self._neon_client.get_holder_account_info(res_info.holder_address)
        ...
        size = self._cfg.holder_size
        balance = self._sol_client.get_rent_exempt_balance_for_size(size)
        builder = NeonIxBuilder(res_info.public_key)
        stage = NeonCreateHolderAccountStage(builder, res_info.holder_address, res_info.holder_seed, size, balance)
        self._execute_stage(stage, res_info)
        print(
            f'Holder account {str(res_info.holder_address)} '
    ...
    def _delete_holder_account(self, args) -> None:
        ...
        builder = NeonIxBuilder(res_info.public_key)
        stage = NeonDeleteHolderAccountStage(builder, res_info.holder_address)
        self._execute_stage(stage, res_info)
        print(
            f'Holder account {str(res_info.holder_address)} '
    ...
    def _find_op_res_by_holder_address(holder_address: SolPubKey) -> Optional[OpResInfo]:
        ...
        for res_info in res_info_list:
            if res_info.holder_address == holder_address:
                return res_info
