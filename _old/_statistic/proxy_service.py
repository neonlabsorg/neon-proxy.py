...
from common_neon import Config
from common_neon.solana import SolPubKey
from common_neon.ethereum.account import NeonAccount

from proxy.neon_core_api.neon_core_api_client import NeonCoreApiClient
from proxy.neon_core_api.neon_layouts import EVMTokenInfo


class ProxyStatDataPeeker(StatDataPeeker):
    def __init__(self, cfg: Config, stat_srv: ProxyStatService):
        super().__init__(cfg, stat_srv)
        self._stat_service = stat_srv
        self._core_api_client = NeonCoreApiClient(cfg)

        self._sol_acct_list: List[SolPubKey] = list()
        self._neon_addr_list: List[NeonAccount] = list()
    ...
    def _stat_operator_balance(self) -> None:
        ...
        sol_balance_list = self._sol_client.get_sol_balance_list(self._sol_acct_list)
        ...
        for neon_acct in neon_acct_list:
            ...
            self._stat_service.commit_op_neon_balance(
                neon_acct.neon_address.to_checksum_address(),
                token_name,
                balance
            )
...
class ProxyStatService(StatService, IHealthStatService):
    def __init__(self, cfg: Config):
        super().__init__(cfg)
        self._data_peeker = ProxyStatDataPeeker(cfg, self)
