...
class NeonIndexerApp:
    ...
    def start(self):
        ...
        self._core_api_service = NeonCoreApiService(self._cfg)
    ...
    def _init_slot_range(self, constants_db: ConstantsDB) -> None:
        solana = SolInteractor(self._cfg)
        ...
        if self._cfg.start_slot == StartSlot.Disable:
            ...
            LOG.debug(f'{self._cfg.start_slot_name}={self._cfg.start_slot}, skip indexing...')
            return
    ...
    def _load_exist_reindex_ranges(self, constants_db: ConstantsDB) -> List[IndexerDB]:
        ...
        for key in constants_db.keys():
            ...
            db = IndexerDB.from_db(self._cfg, DBConnection(self._cfg), db_key)
    ...
    def _get_cfg_reindex_start_slot(self) -> Tuple[Optional[int], str]:
        ...
        if isinstance(reindex_ident, int):
            if reindex_ident >= self._finalized_slot:
                LOG.error(f'{self._cfg.reindex_start_slot_name}={reindex_ident} is too big, skip reindexing...')
                return None, ''
            ...
            if self._cfg.start_slot not in (StartSlot.Latest, StartSlot.Disable):
                LOG.error(
                    f'Wrong value {self._cfg.reindex_start_slot_name}={StartSlot.Continue}, '
                    f'it is valid only for {self._cfg.start_slot_name}=({StartSlot.Latest, StartSlot.Disable}): '
            ...
            LOG.info(
                f'{self._cfg.reindex_start_slot_name}={StartSlot.Continue}: '
                f'started reindexing from the slot: {start_slot}'
            )
            return start_slot, reindex_ident

        LOG.error(f'{self._cfg.reindex_start_slot_name}={reindex_ident}: wrong value, skip reindexing...')
        return None, ''
