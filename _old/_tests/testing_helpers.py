...
from testing.web3 import NeonWeb3
from common_neon.config import Config
from common_neon.config import MIN_FINALIZE_SEC
from common_neon.solana import SolInteractor
from common_neon.solana import SolTx, SolAccount, SolSig, SolPubKey, SolCommit
...
class Proxy:
    ...
    def send_wait_transaction(self, tx: TransactionSigned) -> TransactionSent:
        ...
        neon_tx_hash = self._proxy.send_raw_transaction(tx.tx_signed.rawTransaction)
        tx_receipt = self._proxy.wait_for_transaction_receipt(neon_tx_hash)
    ...
    def compile_and_deploy_contract(self, contract_owner: NeonLocalAccount,
        ...
        return ContractDeployedInfo(contract=contract, neon_tx_hash=tx.neon_tx_hash, tx_receipt=tx.tx_receipt)
    ...
    def get_account_info(self, address: str) -> NeonAccountInfo:
        ...
        return NeonAccountInfo(
            ...
            neon_address=NeonAccount.from_raw(res.address, res.chainId),
...
class SolClient(SolInteractor):
    ...
    def send_tx(self, tx: SolTx, signer: SolAccount, skip_preflight=False) -> Optional[SolSig]:
        ...
        tx.set_recent_blockhash(recent_resp.block_hash)
        tx.sign(signer)
        now = time.time()
