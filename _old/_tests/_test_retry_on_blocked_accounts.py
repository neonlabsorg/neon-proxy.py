...
from common_neon.solana import SolAccountMeta, SolPubKey
from common_neon.solana.solana_tx_legacy import SolLegacyTx
from common_neon import OpResInfo, build_test_resource_info

from proxy.mempool.mempool_executor_task_op_res import OpResInit

from proxy.neon_core_api.neon_client import NeonClient

from testing.solana_utils import wallet_path, WalletAccount
from testing.testing_helpers import Proxy, NeonLocalAccount, SolClient
...
class BlockedTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.solana = sol_client = SolClient(config)

        neon_client = NeonClient(config)

        print("\n_test_retry_on_blocked_accounts.py setUpClass")
        ...
        cls.caller = cls.proxy.get_account_info(cls.neon_account.address).sol_address
    ...
    def create_blocked_transaction(self, resource: OpResInfo):
        ...
        neon_ix_builder.init_operator_neon(
            resource.neon_account_dict[self.chain_id].sol_address
        ).init_neon_tx(
            tx_store.tx_signed.rawTransaction
        ).init_neon_tx_hash(
            NeonTxInfo.from_raw_tx(tx_store.tx_signed.rawTransaction).sig
