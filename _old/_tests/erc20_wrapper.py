...
from common_neon.solana import SolAccountMeta, SolAccount, SolPubKey
from common_neon.config import ACCOUNT_SEED_VERSION, EVM_PROGRAM_ID
from common_neon.ethereum.neon_tx_info import NeonTxInfo
from common_neon.neon_instruction import NeonIxBuilder
from testing.web3 import NeonWeb3, ChecksumAddress, TxParams
...
class ERC20Wrapper:
    ...
    def _create_builder(self, tx: TxParams, owner: SolPubKey, signer_acct: NeonAccount):
        ...
        neon_tx_data = bytearray.fromhex(tx.rawTransaction.hex()[2:])
        emulating_result = self.proxy.neon.neon_emulate(neon_tx_data)
        ...
        account_meta_list = list(neon_account_dict.values())
        account_meta_list.sort(key=lambda x: str(x.pubkey))

        neon = NeonIxBuilder(owner)
        neon.init_operator_neon(
            SolPubKey.from_string(pda_acct)
        ).init_neon_tx(
            neon_tx_data
        ).init_neon_tx_hash(
            NeonTxInfo.from_raw_tx(neon_tx_data).sig
