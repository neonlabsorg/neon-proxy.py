...
from ..common_neon.evm_cfg import EVMConfig
...
from ..neon_core_api.cmd_client import NeonClient
...
def create_transfer_mp_request(*, req_id: str, nonce: int, gas: int, gas_price: int,
    ...
    mp_tx_req = MPTxExecRequest.from_tx_req(
        ...
        res_info=res_info,
        evm_cfg_data=EVMConfig().evm_cfg_data
    )
    return mp_tx_req
...
class TestMemPool(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        stat_client = ProxyStatClient(cfg)
        self._stat_client = stat_client

        user = FakeExecutorMsgUser()
        self._user = user

        self._mempool = MemPool(self._cfg, stat_client)
        self._mempool._op_res_mng = MockResourceManager(self._cfg, stat_client)
        self._mempool._executor_mng = MockMPExecutorMng(cfg, user, stat_client)
        ...
        self._mempool.on_evm_cfg(EVMConfig())
    ...
    async def test_single_sender_couple_txs(self, is_available_mock: MagicMock, submit_mp_request_mock: MagicMock):
        ...
        self._update_state_tx_cnt([
            MPSenderTxCntData(sender=NeonAddress.from_raw(from_acct.address, DEF_CHAIN_ID, neon_tx_hash=), state_tx_cnt=1)
        ])
        self._mempool.on_executor_released(1)
        await asyncio.sleep(self._mempool._check_task_time_sec * 2)
    ...
    async def test_2_senders_4_txs(self, is_available_mock: MagicMock, submit_mp_request_mock: MagicMock):
        ...
        self._update_state_tx_cnt([
            MPSenderTxCntData(sender=NeonAddress.from_raw(acct_list[0].address, DEF_CHAIN_ID, neon_tx_hash=), state_tx_cnt=1),
    ...
    async def test_gas_price_increase(self, is_available_mock: MagicMock, submit_mp_request_mock: MagicMock):
        ...
        nonce_cnt = 101
        capacipy_90pct = self._cfg.mempool_capacity // 10 * 9
    ...
    def _update_state_tx_cnt(self, sender_tx_cnt_list: List[MPSenderTxCntData]) -> None:
        for data in sender_tx_cnt_list:
            tx_pool = self._tx_schedule._find_sender_pool(data.from_address.address)
