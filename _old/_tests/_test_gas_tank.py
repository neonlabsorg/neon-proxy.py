...
from common_neon import Config, StartSlot
from common_neon import NeonTxInfo
...
class TestGasTank(unittest.TestCase):
    ...
    def create_gas_tank(cls, start_slot: Union[str, int]) -> GasTank:
        cfg = FakeConfig(start_slot)
        cls.config = cfg
        return GasTank(config=cfg)

    @classmethod
    def add_neon_pass_analyzer(cls, gas_tank: GasTank):
        neon_pass_whitelist = {neon_pass_erc20_for_spl: neon_pass_gas_less_amount}
        neon_pass_analyzer = NeonPassAnalyzer(gas_tank._cfg, neon_pass_whitelist)
    ...
    def add_portal_analyzer(cls, gas_tank: GasTank):
        ...
        portal_analyzer = PortalAnalyzer(gas_tank._cfg, portal_whitelist)
        gas_tank.add_neon_tx_analyzer(NeonAccount.from_raw(wormhole_contract), portal_analyzer)

    @classmethod
    def add_erc20_analyzer(cls, gas_tank: GasTank):
        erc20_whitelist = {erc20_token_mint: wormhole_gas_less_amount}
        erc20_analyzer = ERC20Analyzer(gas_tank._cfg, erc20_whitelist)
        gas_tank.add_neon_tx_analyzer(NeonAccount.from_raw(wormhole_contract), erc20_analyzer)
    ...
    def test_not_permit_for_already_processed_address(self, mock_add_gas_less_permit, mock_has_gas_less_tx_permit):
        ...
        mock_has_gas_less_tx_permit.assert_called_once_with(NeonAccount.from_raw(neon_pass_gas_less_account))
        mock_add_gas_less_permit.assert_not_called()
    ...
    def test_neon_pass_simple_case(self, mock_allow_gas_less_tx):
        ...
        neon_tx = NeonTxInfo.from_raw_tx(ix_data[5:])

        mock_allow_gas_less_tx.assert_called_once_with(NeonAccount.from_raw(neon_pass_gas_less_account), neon_tx)
    ...
    def test_wormhole_transaction_simple_case(self, mock_allow_gas_less_tx):
        ...
        neon_tx = NeonTxInfo.from_raw_tx(ix_data[41:])

        mock_allow_gas_less_tx.assert_called_once_with(NeonAccount.from_raw(wormhole_gas_less_account), neon_tx)
    ...
    @patch.object(SolInteractor, 'get_slot')
