...
terraform = Terraform(working_dir=pathlib.Path(__file__).parent / "full_test_suite")
...
def check_neon_evm_tag(tag):
    ...
    if response.status_code != 200:
        raise RuntimeError(f"evm_loader image with {tag} tag isn't found. Response: {response.json()}")


def is_branch_exist(branch, repo):
    if branch:
        proxy_branches_obj = requests.get(
            f"https://api.github.com/repos/{GH_ORG_NAME}/{repo}/branches?per_page=100"
        ).json()
...
@click.option(
    "--neon_evm_tag",
    help="the neon evm_loader image tag that will be used for the build",
)
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="skip pulling of docker images from the docker-hub",
)
...
def get_all_containers_logs():
    ...
    subprocess.run(f"ssh-keygen -R {solana_ip} -f {home_path}/.ssh/known_hosts", shell=True)
    subprocess.run(f"ssh-keygen -R {proxy_ip} -f {home_path}/.ssh/known_hosts", shell=True)
    ...
    ssh_client.connect(hostname=solana_ip, username="root", key_filename=ssh_key, timeout=120)

    upload_remote_logs(ssh_client, "solana", artifact_logs)

    ssh_client.connect(hostname=proxy_ip, username="root", key_filename=ssh_key, timeout=120)
...
@click.option(
    ...
    show_default=True,
    default=False,
    help="flag for skipping uniswap tests",
)
@click.option(
    "--test_files",
    help="comma-separated file names if you want to run a specific list of tests",
)
@click.option(
    "--skip_pull",
    is_flag=True,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="mount local dir to the docker"
)
def deploy_check(
    proxy_tag,
    neon_evm_tag,
    ...
    if mount_local:
        ...
        bind_dict = {
            os.path.join(l3_path, d): {
                "bind": os.path.join("/opt/neon-proxy", d),
                "mode": "rw"
            }
            for d in dir_list
        }
...
def get_service_url(project_name: str, service_name: str):
    inspect_out = docker_client.inspect_container(f"{project_name}-proxy-1")
...
def wait_for_service(project_name: str, service_name: str):
    ...
    command = f"docker exec {project_name}-proxy-1 nc -zvw1 {service_ip} {service_port}"
    timeout_sec = 120
...
def run_uniswap_test(project_name):
    ...
    command = f"docker run --rm --network=container:{project_name}-proxy-1 -e {faucet_name}_URL \
...
def trigger_dapps_tests(solana_ip, proxy_ip, pr_url_for_report, token, test_set):
    ...
    wait_condition(
        lambda: github.get_dapps_run_info(run_id)["status"] == "completed",
        timeout_sec=7200,
        delay=5,
    )
